package com.yogeshj.autoform.user.userFormFragment.recommendationRecyclerView
...
class RecommendationCardFormAdapter(private var dataList: ArrayList<RecommendationCardFormModel>, var context: Context) :
    ...
    inner class ViewHolder(var binding: RecommendationCardFormRvItemsBinding) : RecyclerView.ViewHolder(binding.root)
...
//        val db = FirebaseDatabase.getInstance().getReference("LinkApplied")
//        db.addListenerForSingleValueEvent(object : ValueEventListener {
//            override fun onDataChange(snapshot: DataSnapshot) {
//                if (snapshot.exists()) {
//                    for (snap in snapshot.children) {
//                        if(snap.key==FirstScreenActivity.auth.currentUser?.uid){
//                            val curr = snap.getValue(ViewRegisteredModel::class.java)
//                            if (curr != null && dataList[position].examName==curr.name) {
